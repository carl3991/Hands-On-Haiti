...
# https://nbviewer.org/github/carl3991/Hands-On-Haiti/blob/main/FoodEconomy_folium.ipynb
...
# json file location
geojson_path = "/home/af2ea8f5-1f9a-4091-9c13-db2b59ea1801/My Projects/Haiti/ht.json"
...
# Create the base map
haiti_map = folium.Map(location=[19.0, -72.7], zoom_start=8)
...
# The Department whith color black is likely a department that exists in the GeoJSON but doesn't have a matching entry in the median_prices_cleaned dataset. Folium renders unmatched polygons in black when no value is found. Here's a way to fix this...
...
# Let's assign an estimated value for `Nippes` with the overall median of all department.
...
print('The overall median price for the availbale departments is:','$',overall_median)


# Now `Nippes` is assigned the value of 1.6104 us dollars<br> </br>
# Let's visualize the Choropleth again.
...
# Highlighting the 2 High price areas
locs = [19.50853,-73.23007 ] #location to to include the DivIcon text
folium.Marker(
        location=locs,
        icon=DivIcon(html="<div style='font-size:25px; font-weight:bold;color:black;'> High Price Zone</div>")
        ).add_to(haiti_map)
# Display the map
haiti_map
...
# <h1 style="text-align:center; font-size:26px">Summary</h1>
# <p style="text-align:center; font-size:20px">  Haiti’s food markets show significant disparities. Artibonite and North-West region have the highest median prices and heavy reliance on two commodities. TThis uneven distribution shows that some regions have limited access to food, depend heavily on just a few products, and face unstable prices.</p>
